...
@ucrtbase.foreign(ULONG, PEXCEPTION_POINTERS)
...
@ucrtbase.foreign(_crt_app_type)
...
@ucrtbase.foreign(errno_t, _crt_argv_mode)
...
@ucrtbase.foreign(VOID, PTR(_PVFV), PTR(_PVFV))
def _initterm(_First: IPointer[CFuncPtr], 
              _Last: IPointer[CFuncPtr]): ...

@ucrtbase.foreign(VOID, PTR(_PIFV), PTR(_PIFV))
...
@ucrtbase.foreign(INT, _onexit_table_t.PTR(), _onexit_t)
...
@ucrtbase.foreign(BOOL, result_function=bool)
def __acrt_initialize() -> bool: ...

@ucrtbase.foreign(BOOL, BOOL, result_function=bool)
