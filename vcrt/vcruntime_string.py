...
if cpreproc.pragma_once():
    ...
    @msvcrt.foreign(c_void_p, c_void_p, c_int, c_size_t)
    def memchr(_Buf: IVoidPtr, _Val: int, _MaxCount: int) -> IVoidPtr: ...
    
    @msvcrt.foreign(c_int, c_void_p, c_void_p, c_size_t)
    def memcmp(_Buf1: IVoidPtr, _Buf2: IVoidPtr, _Size: int) -> int: ...
    
    @msvcrt.foreign(c_void_p, c_void_p, c_void_p, c_size_t)
